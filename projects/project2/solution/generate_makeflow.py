...
def main():
    ...
    dummy_filename=output_files_dir+'/index_{task_id:04d}_complete'
    dummy_command='touch ' +dummy_filename 
    ...
    with open(makeflow_dir+'/'+song_name+".makeflow", "w") as makefile:
        ...
        # There are way too many .png files to track in makeflow like this
        # Instead we will check for index_XXX_complete files and assume the
        # accompanying images exist

        makefile.write(movie_file+": ")
        for time_index in range(max_time):
                makefile.write(dummy_filename.format(task_id=(time_index)) + " " )
        makefile.write("\n\t" +  ffmpeg_command1 + "\n\n")
    
        # Intermediate step to reduce number of input files required for
        # ffmpeg job

        for time_index in range(max_time):
            makefile.write(dummy_filename.format(task_id=time_index) + ": ") 
            for counter in range(0,60):
                if (time_index*60+counter) >= max_frame :
                    break
                makefile.write(frameName.format(task_id=time_index*60+counter) + " ")
            makefile.write("\n\t"+dummy_command.format(task_id=time_index)+"\n\n") 
        ...
        for time_index in range(max_time):
            ...
            makefile.write("\n\t"+python_command1.format(script_path=rosa_path,flac_file=song_file,start_time=time_index,outdir=output_files_dir) +"\n\n")
