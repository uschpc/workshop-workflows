...
- "image generator"
    ...
    - saved into 1 video file, `output.mp4`
    - Can run in any order
- `ffmpeg`
    - Merges `output.mp4` and `input.flac`
    - creates `final.mp4`
    - Can not run until "image generator" step completes
</div>

## Job type:`rosa_fft.py`

- "image generator" is a python script, `rosa_fft.py`
    -  `python3 rosa_fft.py -f 09Teachers.flac -o 09Teachers`

|option| meaning|
|---|---|
|-f | which song file to read from (doesn't have to be .flac format)|
|-o | which directory to save movie to|


## Job type:`ffmpeg`
- ffmpeg is used to create movie file
    - `ffmpeg -threads 8 -framerate 60 -i 16Funk_Ad/silent.mp4 -i 16Funk_Ad.flac -pix_fmt yuv410p 16Funk_Ad.mp4`
